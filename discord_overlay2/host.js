...
var ipcMain = {
  on: function on(event, callback) {
    return _ipcMain.on('DISCORD_' + event, callback);
  },
  removeListener: function removeListener(event, callback) {
    return _ipcMain.removeListener('DISCORD_' + event, callback);
  }
};
...
function loadOverlay(pid) {
  ...
  renderer.window.webContents.on('ime-composition-range-changed', function (_event, start, end, bounds) {
    Overlay.sendCommand(renderer.pid, { message: 'ime_composition_range_changed', start: start, end: end, bounds: bounds });
  });
  renderer.window.webContents.on('selection-bounds-changed', function (_event, anchor, focus, isAnchorFirst) {
    Overlay.sendCommand(renderer.pid, { message: 'ime_selection_bounds_changed', anchor: anchor, focus: focus, isAnchorFirst: isAnchorFirst });
  });

  renderer.window.webContents.on('did-fail-load', function (e, errCode, errDesc, validatedURL) {
    if (validatedURL !== renderer.url) {
      Overlay.logMessage('Failed non-overlay URL load (' + validatedURL + ') with code ' + errCode + ' and description ' + errDesc);
      return;
    }

    Overlay.logMessage('Failed overlay URL load (' + validatedURL + ') with code ' + errCode + ' and description ' + errDesc);
    Overlay.sendCommand(renderer.pid, { message: 'relay', _relay: 'renderer_load_failed' });
  ...
  renderer.window.on('ready-to-show', function () {
    if (renderer.window.webContents.getURL() === renderer.overlayURL) {
      ...
      Overlay.sendCommand(renderer.pid, { message: 'relay', _relay: 'ready_to_show' });
      renderer.window.webContents.invalidate();
    }
  });

  if (renderer.window.webContents.sendImeEvent) {
    Overlay.sendCommand(renderer.pid, { message: 'notify_ime_supported' });
  }
  ...
  if (event.message === 'graphics_info') {
    if (event.width > 0 && event.height > 0) {
      ...
      if (global.features.supports('overlay-hidpi')) {
        var screenRect = { x: 0, y: 0, width: event.width, height: event.height };
    ...
    if (translated) {
      if (!handleAccelerators(renderer.window.webContents, translated)) {
        if (global.features.supports('overlay-hidpi') && translated.x && translated.y) {
          var dipPoint = _screen.screenToDipPoint({ x: translated.x, y: translated.y });
